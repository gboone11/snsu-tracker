...
class StepExecutionRepository:
    ...
    def get_by_run(self, run_id: int) -> List[Dict[str, Any]]:
        with self.db.get_connection() as conn:
            cursor = conn.execute("SELECT * FROM step_executions WHERE run_id = ?", (run_id,))
            return [dict(row) for row in cursor.fetchall()]

    def get_by_id(self, execution_id: int) -> Optional[Dict[str, Any]]:
        with self.db.get_connection() as conn:
            cursor = conn.execute("SELECT * FROM step_executions WHERE execution_id = ?", (execution_id,))
            row = cursor.fetchone()
    ...
    def update(self, execution_id: int, updates: Dict[str, Any]) -> bool:
        ...
        with self.db.get_connection() as conn:
            cursor = conn.execute(f"UPDATE step_executions SET {set_clause} WHERE execution_id = ?", values)
            return cursor.rowcount > 0

    def delete(self, execution_id: int) -> bool:
        with self.db.get_connection() as conn:
            cursor = conn.execute("DELETE FROM step_executions WHERE execution_id = ?", (execution_id,))
            return cursor.rowcount > 0

...
class ProcessStepRepository:
    ...
    def create(
        ...
        team_name: str,
        task_name: str,
        avg_duration_minutes: Optional[int],
    ) -> int:
    ...
    def get_by_id(self, step_id: int) -> Optional[Dict[str, Any]]:
        with self.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM process_steps WHERE step_id = ?", (step_id,)
            )
            row = cursor.fetchone()
    ...
    def update(self, step_id: int, updates: Dict[str, Any]) -> bool:
        ...
        with self.db.get_connection() as conn:
            cursor = conn.execute(
                f"UPDATE process_steps SET {set_clause} WHERE step_id = ?", values
            )
    ...
    def reorder(self, ordered_ids: List[int]) -> None:
        with self.db.get_connection() as conn:
            for order, step_id in enumerate(ordered_ids, start=1):
                conn.execute(
                    "UPDATE process_steps SET step_order = ? WHERE step_id = ?",
                    (order, step_id),
                )

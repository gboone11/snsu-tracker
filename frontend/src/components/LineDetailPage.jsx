...
function LineDetailPage() {
  ...
  const handleCheckChange = async (stepId, checked) => {
    ...
    if (execution.execution_id) {
      ...
      setExecutions([...executions, newExec.data.data]);
    }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>Add New Process Step</Typography>
        ...
        <Table>
          ...
          <TableBody>
            {steps.map((step, idx) => {
              ...
              const rowColor = status === 'completed' ? '#c8e6c9' : status === 'in_progress' ? '#fff9c4' : 'inherit';
              ...
              return (
                <TableRow key={step.step_id} sx={{ backgroundColor: rowColor }}>
                  ...
                  <TableCell>{execution.status || (idx === 0 ? 'in_progress' : 'not_started')}</TableCell>
                  <TableCell>
                    <TextField
                      size="small"
                      placeholder="MM/DD/YYYY HH:MM AM/PM"
                      value={timeInputs[`${step.step_id}-start_time`] ?? (execution.start_time ? new Date(execution.start_time).toLocaleString() : '')}
                      onChange={(e) => setTimeInputs({ ...timeInputs, [`${step.step_id}-start_time`]: e.target.value })}
                  ...
                  <TableCell>
                    <TextField
                      size="small"
                      placeholder="MM/DD/YYYY HH:MM AM/PM"
                      value={timeInputs[`${step.step_id}-end_time`] ?? (execution.end_time ? new Date(execution.end_time).toLocaleString() : '')}
                      onChange={(e) => setTimeInputs({ ...timeInputs, [`${step.step_id}-end_time`]: e.target.value })}
